...
// Default form fill prompt (fallback if API unavailable)
const DEFAULT_FORM_FILL_PROMPT = `You are filling a job application form using DOM selectors (NOT visual clicking).
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      // Fetch prompt from API with fallback to default
      let prompt: string
      try {
        prompt = await fetchFormFillPrompt()
        logger.info(`[FillForm] Loaded prompt from API (${prompt.length} chars)`)
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err)
        logger.warn(`[FillForm] Failed to fetch prompt from API, using default: ${message}`)
        prompt = DEFAULT_FORM_FILL_PROMPT
      }
