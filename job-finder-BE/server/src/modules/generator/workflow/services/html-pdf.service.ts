          ...
          <div class="experience-list">
            ${content.projects.map((proj) => {
              ...
              const tech = proj.technologies?.length
                ? `<div class="tech">${cleanText(proj.technologies.join(' • '))}</div>`
                : ''
              const link = proj.link
                ? ` • <a href="${normalizeUrl(proj.link)}">${cleanText(proj.link.replace(/^https?:\/\//, '').replace(/\/$/, ''))}</a>`
                : ''
              return `
                <div class="role">
                  <div class="role-header">
                    <span class="role-title">${cleanText(proj.name)}</span>
                  </div>
                  ${link ? `<div class="company">${link.replace(/^ • /, '')}</div>` : ''}
                  ${allBullets ? `<ul class="bullets">${allBullets}</ul>` : ''}
                  ${tech}
                </div>
              `
            }).join('')}
