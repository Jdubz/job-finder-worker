...
describe('DocumentCacheRepository', () => {
  ...
  it('stores and retrieves via exact match', () => {
    ...
    repo.store(TEST_USER, params)

    const result = repo.findExact(TEST_USER, 'fp-hash-1', 'content-hash-1', 'resume')
  ...
  it('findExact returns null on miss', () => {
    repo.store(TEST_USER, makeStoreParams())

    // Wrong fingerprint hash
    expect(repo.findExact(TEST_USER, 'wrong-hash', 'content-hash-1', 'resume')).toBeNull()
    // Wrong content hash
    expect(repo.findExact(TEST_USER, 'fp-hash-1', 'wrong-hash', 'resume')).toBeNull()
    // Wrong document type
    expect(repo.findExact(TEST_USER, 'fp-hash-1', 'content-hash-1', 'cover_letter')).toBeNull()
  })
  ...
  it('findSimilar returns results for matching embedding', () => {
    ...
    repo.store(TEST_USER, makeStoreParams({ embeddingVector: embedding }))

    // Query with the same embedding — should get perfect match
    const results = repo.findSimilar(TEST_USER, embedding, 'content-hash-1', 'resume', 5)
  ...
  it('findSimilar filters by content_items_hash', () => {
    ...
    repo.store(TEST_USER, makeStoreParams({ embeddingVector: embedding, contentItemsHash: 'hash-a' }))

    const results = repo.findSimilar(TEST_USER, embedding, 'hash-b', 'resume', 5)
    expect(results).toHaveLength(0)
  })

  it('findSimilar filters by document_type', () => {
    const embedding = makeEmbedding(3)
    repo.store(TEST_USER, makeStoreParams({ embeddingVector: embedding, documentType: 'resume' }))

    const results = repo.findSimilar(TEST_USER, embedding, 'content-hash-1', 'cover_letter', 5)
    expect(results).toHaveLength(0)
  })
  ...
  it('findSimilar returns results sorted by similarity descending', () => {
    ...
    const results = repo.findSimilar(TEST_USER, emb1, 'content-hash-1', 'resume', 5)
    expect(results.length).toBeGreaterThanOrEqual(1)
  ...
  it('removeStaleEntries removes entries with non-matching hash', () => {
    repo.store(TEST_USER, makeStoreParams({ contentItemsHash: 'old-hash', jobFingerprintHash: 'fp-old', embeddingVector: makeEmbedding(30) }))
    repo.store(TEST_USER, makeStoreParams({ contentItemsHash: 'current-hash', jobFingerprintHash: 'fp-current', embeddingVector: makeEmbedding(31) }))

    const removed = repo.removeStaleEntries(TEST_USER, 'current-hash')

    expect(removed).toBe(1)
    expect(repo.findExact(TEST_USER, 'fp-old', 'old-hash', 'resume')).toBeNull()
    expect(repo.findExact(TEST_USER, 'fp-current', 'current-hash', 'resume')).not.toBeNull()
  })

  it('removeStaleEntries returns 0 when nothing is stale', () => {
    repo.store(TEST_USER, makeStoreParams({ contentItemsHash: 'current-hash' }))
  ...
  it('pruneOlderThan removes old entries', () => {
    repo.store(TEST_USER, makeStoreParams({ jobFingerprintHash: 'fp-old', embeddingVector: makeEmbedding(40) }))
    ...
    // Store a fresh entry
    repo.store(TEST_USER, makeStoreParams({ jobFingerprintHash: 'fp-new', embeddingVector: makeEmbedding(41) }))

    const pruned = repo.pruneOlderThan(30)

    expect(pruned).toBe(1)
    expect(repo.findExact(TEST_USER, 'fp-old', 'content-hash-1', 'resume')).toBeNull()
    expect(repo.findExact(TEST_USER, 'fp-new', 'content-hash-1', 'resume')).not.toBeNull()
  })
  ...
  it('findByRoleFingerprint returns cached entry for same role across companies', () => {
    ...
    const result = repo.findByRoleFingerprint(TEST_USER, 'role-fp-1', 'content-hash-1', 'resume')
    expect(result).not.toBeNull()
  ...
  it('findByRoleFingerprint returns null on miss', () => {
    repo.store(TEST_USER, makeStoreParams({ roleFingerprintHash: 'role-fp-1' }))

    expect(repo.findByRoleFingerprint(TEST_USER, 'wrong-role-fp', 'content-hash-1', 'resume')).toBeNull()
    expect(repo.findByRoleFingerprint(TEST_USER, 'role-fp-1', 'wrong-hash', 'resume')).toBeNull()
    expect(repo.findByRoleFingerprint(TEST_USER, 'role-fp-1', 'content-hash-1', 'cover_letter')).toBeNull()
  })
  ...
  it('findByRoleFingerprint returns a valid entry when multiple share the same role fingerprint', () => {
    ...
    const result = repo.findByRoleFingerprint(TEST_USER, 'role-fp-shared', 'content-hash-1', 'resume')
    expect(result).not.toBeNull()
  ...
  it('store replaces existing entry with same fingerprint instead of duplicating', () => {
    repo.store(TEST_USER, makeStoreParams({ documentContentJson: JSON.stringify({ v: 1 }) }))
    repo.store(TEST_USER, makeStoreParams({ documentContentJson: JSON.stringify({ v: 2 }) }))
  ...
  it('findExact does not update hit metadata (non-mutating)', () => {
    repo.store(TEST_USER, makeStoreParams())
    repo.findExact(TEST_USER, 'fp-hash-1', 'content-hash-1', 'resume')
    repo.findExact(TEST_USER, 'fp-hash-1', 'content-hash-1', 'resume')
  ...
  it('recordHit increments hit count', () => {
    repo.store(TEST_USER, makeStoreParams())
    const hit = repo.findExact(TEST_USER, 'fp-hash-1', 'content-hash-1', 'resume')
  ...
  it('stores and retrieves cover_letter_body type', () => {
    ...
    const result = repo.findExact(TEST_USER, 'fp-body-1', 'content-hash-1', 'cover_letter_body')
    expect(result).not.toBeNull()
  ...
  it('findByRoleFingerprint retrieves cover_letter_body entries', () => {
    ...
    const result = repo.findByRoleFingerprint(TEST_USER, 'role-fp-body-2', 'content-hash-1', 'cover_letter_body')
    expect(result).not.toBeNull()
    expect(JSON.parse(result!.documentContentJson)).toEqual(bodyParagraphs)
  })
  ...
  it('cover_letter_body type does not collide with cover_letter type', () => {
    ...
    expect(repo.findExact(TEST_USER, 'fp-clb-1', 'content-hash-1', 'cover_letter')).toBeNull()
    // And vice versa
    expect(repo.findExact(TEST_USER, 'fp-cl-1', 'content-hash-1', 'cover_letter_body')).toBeNull()
    // But each should find its own type
    expect(repo.findExact(TEST_USER, 'fp-cl-1', 'content-hash-1', 'cover_letter')).not.toBeNull()
    expect(repo.findExact(TEST_USER, 'fp-clb-1', 'content-hash-1', 'cover_letter_body')).not.toBeNull()
  })
  ...
  it('findByArchetypeFingerprint returns cached entry for same archetype', () => {
    ...
    const result = repo.findByArchetypeFingerprint(TEST_USER, 'arch-fp-1', 'content-hash-1', 'resume')
    expect(result).not.toBeNull()
    expect(result!.companyName).toBe('Acme Corp')
    expect(JSON.parse(result!.documentContentJson)).toEqual({ summary: 'Test resume content' })
  })
  ...
  it('findByArchetypeFingerprint returns null on miss', () => {
    repo.store(TEST_USER, makeStoreParams({ archetypeFingerprintHash: 'arch-fp-1' }))

    expect(repo.findByArchetypeFingerprint(TEST_USER, 'wrong-arch-fp', 'content-hash-1', 'resume')).toBeNull()
    expect(repo.findByArchetypeFingerprint(TEST_USER, 'arch-fp-1', 'wrong-hash', 'resume')).toBeNull()
    expect(repo.findByArchetypeFingerprint(TEST_USER, 'arch-fp-1', 'content-hash-1', 'cover_letter')).toBeNull()
  })
  ...
  it('findByArchetypeFingerprint returns a valid entry when multiple share the same archetype fingerprint', () => {
    ...
    const result = repo.findByArchetypeFingerprint(TEST_USER, 'arch-fp-shared', 'content-hash-1', 'resume')
    expect(result).not.toBeNull()
    expect(['Corp A', 'Corp B']).toContain(result!.companyName)
  })
