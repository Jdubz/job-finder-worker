...
2. Remove legacy rows (`title-filter`, `queue-settings`, `scoring-config`, `scheduler-settings`, `job-match`, `job-filters`, `stop-list`, `technology-ranks`) and migrate data into canonical keys; add one-time migrator that deletes old rows after backup.
