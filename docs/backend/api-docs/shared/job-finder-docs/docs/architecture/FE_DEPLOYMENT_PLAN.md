# Frontend Public Deployment Plan

**Status**: Drafted 2025-10-19  
**Owner**: PM  
**Scope**: Make `job-finder-FE` publicly accessible on custom Cloudflare-managed domains with automated deploys for staging and production.
...
1. **Week 1** – Terraform module draft, service accounts, secrets migration.
2. **Week 2** – DNS + Firebase custom domain integration, CI/CD hardening, preview deploys.
3. **Week 3** – Monitoring, documentation, launch rehearsal, go-live.
...
- [ ] PM: Align with stakeholders on domains and compliance requirements.
- [ ] Worker B: Inventory all environment secrets and identify sensitive values stored in git.
- [ ] PM: Draft Terraform module skeleton and secret management strategy.
- [ ] Worker B: Prototype preview channel deploy from staging branch to validate workflows.
