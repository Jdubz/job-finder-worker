# Frontend Dev Server Startup Guide
...
This guide covers automated testing to verify the frontend dev server (`npm run dev`) starts reliably, runs on the expected port, and shuts down gracefully. These procedures ensure the dev-monitor can reliably control the frontend service.

## Context

The dev-monitor app manages the frontend dev server as one of its controlled services. This guide ensures:
...
None - frontend dev server can start immediately.
