...
✅ Use `make dev-ui` for full stack development
✅ Use `make dev-health-check` before debugging
✅ Commit `.env.example` files, never `.env`
✅ Run `make dev-validate-env` after updating tools
✅ Use tmux pane zoom (`Ctrl+B z`) for focused work
✅ Check `make dev-status` before starting services

### DON'T

❌ Don't manually start services in different terminals
❌ Don't use `sleep` for timing - use health checks
...
- No external dependency (tmux is standard)
- Health checks ensure proper startup order
- Consistent commands across all repos
- Better process management
- Easier debugging with individual service control
...
- [ ] Docker Compose orchestration option
