...
SELECT
  ...
  COALESCE(
    ...
    json_extract(body_json, '$.position'),
    NULL
  ) AS role,
  json_extract(body_json, '$.location') AS location,
  json_extract(body_json, '$.website') AS website,
  json_extract(body_json, '$.startDate') AS start_date,
  json_extract(body_json, '$.endDate') AS end_date,
  json_extract(body_json, '$.description') AS description,
  json_extract(body_json, '$.skills') AS skills,
