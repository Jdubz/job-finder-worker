-- Add 'resume_body' document type for sectional resume caching.
-- Mirrors the cover_letter_body pattern: cache experience/projects/skills/education
-- by role fingerprint, then only regenerate title + summary per company.
-- SQLite doesn't support ALTER CHECK, so we recreate the table.
-- Preserves all existing data and indexes (including archetype_fingerprint_hash from 055).
...
DROP INDEX IF EXISTS idx_document_cache_archetype_fingerprint;
...
CREATE TABLE document_cache (
    ...
    document_type TEXT NOT NULL CHECK (document_type IN ('resume', 'cover_letter', 'cover_letter_body', 'resume_body')),
    ...
    archetype_fingerprint_hash TEXT
);
...
INSERT INTO document_cache (
    ...
    company_name, hit_count, last_hit_at, model_version, created_at,
    role_fingerprint_hash, archetype_fingerprint_hash
)
SELECT
    id, embedding_rowid, document_type, job_fingerprint_hash, content_items_hash,
    role_normalized, tech_stack_json, document_content_json, job_description_text,
...
CREATE INDEX idx_document_cache_archetype_fingerprint ON document_cache(archetype_fingerprint_hash, content_items_hash, document_type);
